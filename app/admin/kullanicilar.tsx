...
export default function AdminUserListView({
    ...
    const loadData = async (page: number = 1, isRefresh: boolean = false) => {
        try {
            ...
            const response = await userApiService.getUsers(1);
    ...
    useEffect(() => {
        loadData().then(r => console.log("admin user load data", r));
    }, []);
    ...
    const validateForm = () => {
        if (!formData.identityNumber || formData.identityNumber.length !== 11) {
            Alert.alert('Hata', 'Kimlik numarası 11 haneli olmalıdır!');
            return false;
        }
        ...
        if (!formData.job) {
            Alert.alert('Hata', 'Meslek alanı boş bırakılamaz!');
            return false;
        }
    ...
    const handleSave = async () => {
        try {
            ...
            const dataToSave = {
                ...
                job: formData.job,
                avatar: selectedImage?.base64 || formData.avatar,
            };

            if (editMode && selectedItem != null) {
                const updateData: Partial<UpdateUserRequest> = dataToSave;
                await userApiService.updateUser(selectedItem.id, updateData);
            } else {
    ...
    const handleDelete = async (item: UserDto) => {
        Alert.alert('Kullanıcıyı Sil', 'Bu kullanıcıyı silmek istediğinize emin misiniz?', [
            { text: 'İptal', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        await userApiService.deleteUser(item.id);
                        await loadData();
                    } catch (e) {
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                <DKTextInput
                    ...
                    onChange={(text) => setFormData({ ...formData, identityNumber: text })}
                ...
                >
                    <Text style={{ color: 'white' }}>Profil Fotoğrafı Seç</Text>
                </TouchableOpacity>

                {(formData.avatar || selectedImage) && (
                    <Image
                        source={{
                            uri: selectedImage ? selectedImage.uri : `${BASE_STORAGE_URL}${formData.avatar}`
                        }}
